...
var wrapWASI = (wasiObject, wasmModule) => {
  ...
  for (const importEntry of WebAssembly.Module.imports(wasmModule)) {
    ...
    if (importModule !== "wasi_snapshot_preview1" || importKind !== "function" || wasiObject.wasiImport[importName]) {
      continue;
    }
...
var startWasiTask = async () => {
  const response = await fetch("4f6363df443ae381.wasm");
